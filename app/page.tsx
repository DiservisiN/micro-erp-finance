...
export default function Home() {
  ...
  const PieTooltip = ({ active, payload }: any) => {
    ...
    return (
      <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm shadow-lg">
        <p className="font-medium text-slate-800 dark:text-white">{payload[0].name}</p>
        <p className="text-slate-600 dark:text-slate-300 font-bold">{formatRupiah(payload[0].value)}</p>
      </div>
    );
  };
  ...
  const BarTooltip = ({ active, payload, label }: any) => {
    if (!active || !payload?.length) return null;
    return (
      <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm px-4 py-3 text-sm shadow-xl min-w-[180px] z-50">
        <p className="mb-2 font-semibold text-slate-800 dark:text-white border-b border-slate-200 dark:border-slate-700 pb-1">Tanggal {label}</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] p-4 md:p-6 lg:p-8 transition-colors duration-300">
      <div className="max-w-7xl mx-auto space-y-6">
        
        {/* HEADER & MONTH PICKER */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900/40 p-4 rounded-xl border border-slate-200 dark:border-slate-800/50 shadow-sm dark:shadow-none">
          <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-white text-sm rounded-md px-3 py-1.5 focus:ring-2 focus:ring-orange-500 outline-none cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2 bg-white dark:bg-slate-900/50 backdrop-blur-sm border-slate-200 dark:border-slate-800/50 shadow-sm dark:shadow-xl">
            <CardHeader className="pb-2 border-b border-slate-200 dark:border-slate-800/50">
            ...
            <CardContent className="pt-6">
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={barDataDaily} margin={{ top: 5, right: 10, left: 20, bottom: 5 }}>
                  ...
                  <XAxis dataKey="day" tick={{ fill: isDark ? "#64748b" : "#94a3b8", fontSize: 11 }} axisLine={{ stroke: isDark ? "#334155" : "#cbd5e1" }} tickLine={false} />
                  <YAxis tick={{ fill: isDark ? "#64748b" : "#94a3b8", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(v: number) => shortCurrency(v)} />
                  <Tooltip content={<BarTooltip />} cursor={{ fill: isDark ? "#1e293b" : "#f1f5f9", opacity: 0.5 }} />
                  <Legend wrapperStyle={{ paddingTop: "20px", fontSize: "12px", color: isDark ? "#cbd5e1" : "#64748b" }} />
          ...
          <Card className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border-slate-200 dark:border-slate-800/50 shadow-sm dark:shadow-xl flex flex-col">
            <CardHeader className="pb-2 border-b border-slate-200 dark:border-slate-800/50">
              <CardTitle className="text-slate-800 dark:text-white text-lg">Asset Distribution</CardTitle>
            </CardHeader>
              ...
              <div className="w-full grid grid-cols-2 gap-x-2 gap-y-3 mt-4">
                {pieData.map((entry, index) => (
                  <div key={index} className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-600 dark:text-slate-300 truncate" title={entry.name}>{entry.name}</span>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border-slate-200 dark:border-slate-800/50 shadow-sm dark:shadow-xl">
            <CardHeader className="pb-2 border-b border-slate-200 dark:border-slate-800/50">
              <CardTitle className="text-slate-800 dark:text-white text-lg">6-Month Trend Overview</CardTitle>
            </CardHeader>
            <CardContent className="pt-6">
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={barDataMonthly} margin={{ top: 5, right: 5, left: 10, bottom: 5 }}>
                  ...
                  <YAxis tick={{ fill: isDark ? "#64748b" : "#94a3b8", fontSize: 12 }} axisLine={false} tickLine={false} tickFormatter={shortCurrency} />
                  <Tooltip content={<BarTooltip />} cursor={{ fill: isDark ? "#1e293b" : "#f1f5f9", opacity: 0.5 }} />
          ...
          <Card className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border-slate-200 dark:border-slate-800/50 shadow-sm dark:shadow-xl overflow-hidden flex flex-col">
            <CardHeader className="pb-2 border-b border-slate-200 dark:border-slate-800/50">
              <CardTitle className="text-slate-800 dark:text-white text-lg">Recent Activities ({MONTHS[selectedMonth]})</CardTitle>
            </CardHeader>
            <CardContent className="p-0 flex-1 flex flex-col justify-between">
              <div className="overflow-x-auto">
                {recentTransactions.length === 0 ? (
                  <div className="flex-1 flex items-center justify-center py-12">
                    <p className="text-slate-500 dark:text-slate-400 text-sm">Tidak ada transaksi di bulan ini.</p>
                  </div>
                ) : (
                  <Table className="min-w-[400px]">
                    <TableHeader className="bg-slate-50 dark:bg-slate-950/50">
                      <TableRow className="border-slate-200 dark:border-slate-800/50">
                        <TableHead className="text-slate-500 dark:text-slate-400 font-medium h-10">Date</TableHead>
                        <TableHead className="text-slate-500 dark:text-slate-400 font-medium h-10">Category</TableHead>
                        <TableHead className="text-slate-500 dark:text-slate-400 font-medium text-right h-10">Amount</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {recentTransactions.map((tx) => {
                        ...
                        return (
                          <TableRow key={tx.id} className="border-slate-200 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/40">
                            <TableCell className="text-slate-700 dark:text-slate-300 text-sm py-3 whitespace-nowrap">{formatDate(tx.date)}</TableCell>
                            <TableCell className="py-3">
                              <div className="flex flex-col gap-0.5">
                                <span className="text-slate-800 dark:text-slate-200 text-sm font-medium whitespace-nowrap">{tx.category || formatTransactionType(tx.type)}</span>
                              </div>
                            </TableCell>
                            <TableCell className={`text-right font-mono font-medium text-sm py-3 whitespace-nowrap ${isIncome ? "text-emerald-500 dark:text-emerald-400" : isExpense ? "text-red-500 dark:text-red-400" : "text-slate-500 dark:text-slate-400"}`}>
...
function StatCard({ title, value, subtitle, accent }: { title: string, value: string, subtitle: string, accent: "orange" | "blue" | "green" | "red" }) {
  ...
  return (
    <Card className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border-slate-200 dark:border-slate-800/50 hover:border-slate-300 dark:hover:border-slate-700/80 transition-colors shadow-sm dark:shadow-lg relative overflow-hidden group">
      <div className={`absolute top-0 right-0 w-32 h-32 opacity-0 group-hover:opacity-5 dark:group-hover:opacity-10 blur-3xl rounded-full transition-opacity duration-500 bg-current ${accentColors[accent]}`}></div>
...
function NetProfitCard({ netProfit, monthName }: { netProfit: number, monthName: string }) {
  ...
  return (
    <Card className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border-slate-200 dark:border-slate-800/50 hover:border-slate-300 dark:hover:border-slate-700/80 transition-colors shadow-sm dark:shadow-lg relative overflow-hidden group">
      <div className={`absolute top-0 right-0 w-32 h-32 opacity-0 group-hover:opacity-5 dark:group-hover:opacity-10 blur-3xl rounded-full transition-opacity duration-500 bg-current ${accentColor}`}></div>
      <CardContent className="p-5 relative z-10">
        <div className="flex items-center justify-between mb-1">
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Net Profit</p>
          <div className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${isPositive ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400" : "bg-red-100 text-red-600 dark:bg-red-500/10 dark:text-red-400"}`}>
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500">In {monthName}</p>
      </CardContent>
    </Card>
  );
}
...
function DebtTableCard({ title, items, accent, emptyMsg }: { title: string, items: Debt[], accent: "green" | "red", emptyMsg: string }) {
  const accentColor = accent === "green" ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400";
  const badgeColor = accent === "green" ? "bg-emerald-50 border-emerald-200 dark:bg-emerald-500/10 dark:border-emerald-500/20" : "bg-red-50 border-red-200 dark:bg-red-500/10 dark:border-red-500/20";
  const totalAmount = items.reduce((sum: number, item: Debt) => sum + item.amount, 0);

  return (
    <Card className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border-slate-200 dark:border-slate-800/50 shadow-sm dark:shadow-xl overflow-hidden flex flex-col">
      <CardHeader className="pb-2 border-b border-slate-200 dark:border-slate-800/50 flex flex-row items-center justify-between">
        <CardTitle className="text-slate-800 dark:text-white text-lg flex items-center gap-2"><Users className="h-5 w-5 text-slate-400" />{title}</CardTitle>
        <span className={`font-mono font-bold text-sm ${accentColor}`}>{formatRupiah(totalAmount)}</span>
      </CardHeader>
      <CardContent className="p-0 flex-1">
        {items.length === 0 ? (
          <div className="flex items-center justify-center py-8"><p className="text-slate-500 dark:text-slate-400 text-sm">{emptyMsg}</p></div>
        ) : (
          <div className="max-h-[250px] overflow-y-auto">
            <Table className="min-w-[300px]">
              <TableBody>
                {items.map((item: Debt) => (
                  <TableRow key={item.id} className="border-slate-200 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/40">
                    <TableCell className="py-3">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-slate-800 dark:text-slate-200 text-sm font-medium">{item.personName}</span>
                        <span className="text-xs text-slate-500 dark:text-slate-500 truncate max-w-[200px]">{item.notes || "-"}</span>
                      </div>
                    </TableCell>
...
function BalanceSheetView() {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 md:gap-6 w-full mt-6">
      <div className="bg-white dark:bg-slate-900/40 backdrop-blur-sm border border-slate-200 dark:border-slate-800/50 rounded-xl p-5 md:p-6 w-full h-fit shadow-sm dark:shadow-none">
        <h3 className="text-base md:text-lg font-semibold text-slate-800 dark:text-white mb-4 border-b border-slate-200 dark:border-slate-800/50 pb-2">Harta (Aktiva)</h3>
      ...
      <div className="flex flex-col gap-4 md:gap-6 w-full">
        <div className="bg-white dark:bg-slate-900/40 backdrop-blur-sm border border-slate-200 dark:border-slate-800/50 rounded-xl p-5 md:p-6 w-full h-fit shadow-sm dark:shadow-none">
          <h3 className="text-base md:text-lg font-semibold text-slate-800 dark:text-white mb-4 border-b border-slate-200 dark:border-slate-800/50 pb-2">Hutang (Kewajiban)</h3>
          <div className="space-y-3">
            <div className="flex justify-between items-center text-xs md:text-sm">
            ...
            <div className="flex justify-between items-center pt-3 border-t border-slate-200 dark:border-slate-800/50 mt-2">
              <span className="font-semibold text-red-600 dark:text-red-400 text-xs md:text-sm">Total Hutang</span>
        ...
        <div className="bg-orange-50/50 dark:bg-slate-900/40 backdrop-blur-sm border border-orange-200 dark:border-orange-500/30 ring-1 ring-orange-100 dark:ring-orange-500/10 rounded-xl p-5 md:p-6 shadow-sm dark:shadow-[0_0_15px_rgba(249,115,22,0.05)] w-full h-fit">
          <h3 className="text-base md:text-lg font-semibold text-slate-800 dark:text-white mb-4 border-b border-orange-200 dark:border-slate-800/50 pb-2">Modal (Ekuitas)</h3>
          <div className="space-y-3">
            <div className="flex justify-between items-center text-xs md:text-sm">
            ...
            <div className="flex justify-between items-center pt-3 border-t border-orange-200 dark:border-slate-800/50 mt-2">
              <span className="font-semibold text-orange-600 dark:text-orange-400 text-xs md:text-sm">Total Pasiva (Hutang + Modal)</span>
