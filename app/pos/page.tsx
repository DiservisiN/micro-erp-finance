...
import { Search, ShoppingCart, Plus, Minus, Trash2, PackageOpen, Store, Zap, Camera, ChevronDown, Printer, CheckCircle2 } from "lucide-react";
...
export default function POSPage() {
  ...
  // STATE: Batas Tampilan & Struk Belanja
  ...
  const handleCheckoutRetail = async () => {
    ...
    try {
      ...
      setReceiptData({
        transactionId: "TRX-" + generatedId.slice(-6),
        date: new Date().toLocaleString("id-ID"),
        items: [...cart],
        total: totalAmount,
        method: paymentMethod,
        customer: personName.trim() || "Pelanggan Tunai"
      });
  ...
  const handleServiceSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    // Variabel untuk menyusun struk jasa
    let rItems: CartItem[] = [];
    let rTotal = 0;
    let rCustomer = personName.trim() || "Pelanggan Tunai";
    ...
    if (transactionType === "Money Transfer" || transactionType === "Cash Withdrawal") {
      ...
      // Susun Struk
      rItems.push({ productId: "tx1", name: transactionType, price: pAmount, quantity: 1, maxStock: 1 });
      if (pAdminFee > 0) rItems.push({ productId: "tx2", name: "Biaya Admin", price: pAdminFee, quantity: 1, maxStock: 1 });
      rTotal = pAmount + pAdminFee;
      ...
      // Susun Struk
      rItems.push({ productId: "tx1", name: `Transfer ke ${selDestWallet.name}`, price: pAmount, quantity: 1, maxStock: 1 });
      rTotal = pAmount;
      rCustomer = "Internal System";
      ...
      rItems.push({ productId: "svc1", name: "Biaya Jasa Service", price: pServiceFee, quantity: 1, maxStock: 1 });
      ...
      if (selSparepart) {
        ...
        rItems.push({ productId: "svc2", name: `Part: ${selSparepart.name}`, price: selSparepart.sellingPrice, quantity: 1, maxStock: 1 });
      }
      ...
      // Susun Struk
      rItems.push({ productId: "ppob1", name: ppobProductName.trim(), price: pPpobSelling, quantity: 1, maxStock: 1 });
      rTotal = pPpobSelling;
      ...
      // Susun Struk
      rItems.push({ productId: "aff1", name: `Komisi ${platformName.trim()}`, price: pCommission, quantity: 1, maxStock: 1 });
      rTotal = pCommission;
      rCustomer = personName.trim() || "Platform Affiliate";
      ...
      // Susun Struk
      rItems.push({ productId: "net1", name: "Tagihan Internet WiFi", price: pInternetAmount, quantity: 1, maxStock: 1 });
      rTotal = pInternetAmount;
      rCustomer = customerName.trim();
      ...
      // Susun Struk
      rItems.push({ productId: "emp1", name: "Penarikan Kasbon", price: pAmount, quantity: 1, maxStock: 1 });
      rTotal = pAmount;
      rCustomer = employeeName.trim();
    }

    toast.success("Transaksi Jasa/Layanan berhasil dicatat!");
    
    // Tampilkan Struk Jasa
    setReceiptData({
      transactionId: "SVC-" + newTxId.slice(-6),
      date: new Date().toLocaleString("id-ID"),
      items: rItems,
      total: rTotal,
      method: transactionType === "Balance Transfer" ? "lunas" : paymentMethod,
      customer: rCustomer
    });
  ...
  return (
    <>
      {/* LOGIKA CSS PINTAR UNTUK PRINTING (Hanya berlaku saat mencetak) */}
      ...
      {/* =====================================================================
          DIALOG PRATINJAU STRUK & KODE HTML STRUK YANG AKAN DICETAK
          ===================================================================== */}
      <Dialog open={!!receiptData} onOpenChange={(open) => !open && closeReceiptDialog()}>
        <DialogContent className="sm:max-w-md bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white rounded-xl shadow-xl print:hidden">
          <DialogHeader className="flex flex-col items-center justify-center pt-6 pb-2">
            ...
            <DialogTitle className="text-xl font-bold">Transaksi Sukses!</DialogTitle>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Struk siap dicetak untuk pelanggan.</p>
          </DialogHeader>

          {/* PRATINJAU STRUK DI LAYAR (Hanya Preview, bukan yang dicetak) */}
          <div className="bg-white text-black p-4 rounded-md border border-dashed border-slate-300 mx-auto w-[250px] font-mono text-[10px] shadow-sm my-4 flex flex-col gap-2 relative">
             <div className="text-center border-b border-dashed border-black pb-2 mb-1">
               <h3 className="font-bold text-sm">MICRO ERP STORE</h3>
               <p>Jakarta, Indonesia</p>
               <p>{receiptData?.date}</p>
             </div>
             <p>No: {receiptData?.transactionId}</p>
             <p>Pelanggan: {receiptData?.customer}</p>
             <div className="border-b border-dashed border-black pb-2 mb-1">
               {receiptData?.items.map((item, i) => (
                 <div key={i} className="flex justify-between items-start mt-1">
                   <div className="flex-1 pr-1">
                     <p className="truncate w-[120px]">{item.name}</p>
                     <p>{item.quantity} x {item.price}</p>
                   </div>
                   <p className="font-bold text-right pt-3">{item.price * item.quantity}</p>
                 </div>
               ))}
             </div>
             <div className="flex justify-between font-bold text-xs mt-1">
                <p>TOTAL:</p>
                <p>{formatRupiah(receiptData?.total || 0)}</p>
             </div>
             <div className="flex justify-between mt-1">
                <p>Metode:</p>
                <p className="uppercase">{receiptData?.method}</p>
             </div>
          ...
          <DialogFooter className="flex-row justify-center gap-3 sm:justify-center">
            <Button type="button" variant="outline" onClick={closeReceiptDialog} className="flex-1 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300">
              Tutup & Lanjut
            </Button>
            <Button type="button" onClick={handlePrintReceipt} className="flex-1 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white shadow-md">
      ...
      {/* ELEMEN HTML ASLI YANG AKAN DICETAK OLEH PRINTER THERMAL (Sembunyi di Layar Biasa) */}
      {receiptData && (
        <div id="printable-receipt" className="hidden text-black font-mono text-[11px] leading-tight bg-white">
           <div style={{ textAlign: 'center', marginBottom: '8px', paddingBottom: '4px', borderBottom: '1px dashed black' }}>
             <h2 style={{ fontWeight: 'bold', fontSize: '14px', margin: 0 }}>MICRO ERP STORE</h2>
             <p style={{ margin: 0 }}>Jakarta, Indonesia</p>
             <p style={{ margin: 0 }}>{receiptData.date}</p>
           </div>
           <p style={{ margin: 0 }}>No: {receiptData.transactionId}</p>
           <p style={{ margin: 0, marginBottom: '4px' }}>Cst: {receiptData.customer}</p>
           ...
           <div style={{ borderBottom: '1px dashed black', paddingBottom: '4px', marginBottom: '4px' }}>
             {receiptData.items.map((item, i) => (
               <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                 <div>
                   <div style={{ maxWidth: '140px', overflow: 'hidden', whiteSpace: 'nowrap' }}>{item.name}</div>
                   <div>{item.quantity} x {item.price}</div>
                 </div>
                 <div style={{ fontWeight: 'bold', alignSelf: 'flex-end' }}>{item.price * item.quantity}</div>
               </div>
             ))}
           </div>
           ...
           <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>Metode:</span>
              <span style={{ textTransform: 'uppercase' }}>{receiptData.method}</span>
           </div>
           
           <div style={{ textAlign: 'center', marginTop: '12px', paddingTop: '4px', borderTop: '1px dashed black' }}>
      ...
      try {
        await html5QrCode.start(
          ...
          (decodedText) => {
            ...
            setTimeout(() => { hasDetectedRef.current = false; }, 2000); 
          },
          () => {}, 
        );
      } catch (error) {
