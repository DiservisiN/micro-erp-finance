...
import { Search, ShoppingCart, Plus, Minus, Trash2, CreditCard, PackageOpen, Store, Zap } from "lucide-react";
...
export default function POSPage() {
  ...
  // DOKUMENTASI: Fungsi baru untuk mengubah harga barang secara manual (Tawar-menawar) di keranjang
  ...
  // Total akan otomatis terhitung dengan benar menggunakan harga baru yang kamu ketik
  ...
  const handleCheckoutRetail = async () => {
    ...
    try {
      // DOKUMENTASI: Menyimpan catatan transaksi dengan mencatat harga kesepakatan per barang
  ...
  const handleServiceSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (transactionType === "Money Transfer" || transactionType === "Cash Withdrawal") {
      ...
      if (paymentMethod === "lunas" && selDestWallet) {
        ...
        await addTransaction({
          id: newTxId, type: "expense", category: isTransfer ? "Kasbon Transfer" : "Kasbon Tarik Tunai", amount: pAmount, fromWalletId: selSourceWallet.id, date: getIsoDate(), notes: `Modal kasbon untuk ${personName}`
        });
  ...
  return (
    <div className="flex flex-col w-full gap-4 md:gap-6 bg-[#020617] min-h-screen p-4 md:p-6 overflow-x-hidden">
      ...
      {/* --- HEADER TABS --- */}
      <div className="w-full bg-slate-900/50 backdrop-blur-sm border border-slate-800/50 rounded-xl p-4 md:p-6 shadow-sm">
        <h2 className="text-xl md:text-2xl font-bold text-white mb-4">Command Center / POS</h2>
        <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as "retail" | "services")} className="w-full">
          <TabsList className="grid w-full grid-cols-2 bg-slate-800/50 border border-slate-700/50 rounded-lg p-1">
            <TabsTrigger value="retail" className="data-[state=active]:bg-orange-500 data-[state=active]:text-white text-slate-400 font-medium py-2 rounded-md transition-all flex items-center gap-2">
              <Store className="h-4 w-4" /> Retail & Barang
            </TabsTrigger>
            <TabsTrigger value="services" className="data-[state=active]:bg-emerald-500 data-[state=active]:text-white text-slate-400 font-medium py-2 rounded-md transition-all flex items-center gap-2">
      ...
      {activeTab === "retail" && (
        <div className="flex flex-col lg:flex-row w-full gap-4 md:gap-6">
          <div className="flex-1 flex flex-col gap-4">
            <div className="bg-slate-900/40 backdrop-blur-sm border border-slate-800/50 rounded-xl p-4">
              <div className="relative w-full">
                ...
                <Input type="text" placeholder="Cari produk di gudang..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 bg-slate-800/50 border-slate-700/50 text-white focus:ring-orange-500/50" />
              </div>
            </div>
            <div className="bg-slate-900/20 rounded-xl p-2 md:p-4 border border-slate-800/30 flex-1 min-h-[400px] overflow-y-auto custom-scrollbar">
              {availableProducts.length === 0 ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
                  {availableProducts.map((product) => (
                    <div key={product.id} onClick={() => addToCart(product)} className="bg-slate-900/60 border border-slate-700/50 rounded-lg p-3 md:p-4 cursor-pointer hover:border-orange-500/50 hover:bg-slate-800/80 transition-all flex flex-col justify-between group active:scale-95 shadow-sm">
                      <div>
                        <h3 className="text-white font-medium text-sm leading-tight mb-1 group-hover:text-orange-400 transition-colors line-clamp-2">{product.name}</h3>
                        <p className="text-[10px] md:text-xs text-slate-400 mb-3 bg-slate-800/80 w-fit px-1.5 py-0.5 rounded">Stok: {product.stock}</p>
                      </div>
                      <div className="font-mono font-bold text-emerald-400 text-sm">{formatRupiah(product.sellingPrice)}</div>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="w-full lg:w-[350px] xl:w-[400px] flex flex-col gap-4">
            <div className="bg-slate-900/40 backdrop-blur-sm border border-slate-800/50 rounded-xl p-4 md:p-6 flex flex-col h-[calc(100vh-8rem)] sticky top-6">
              <div className="flex items-center gap-2 mb-4 pb-4 border-b border-slate-800/50">
                ...
                <span className="ml-auto bg-orange-500/20 text-orange-400 py-0.5 px-2.5 rounded-full text-xs font-bold">{cart.reduce((sum, item) => sum + item.quantity, 0)}</span>
              </div>
              <div className="flex-1 overflow-y-auto space-y-3 pr-1 custom-scrollbar">
                {cart.length === 0 ? (
                  <div className="text-center text-slate-500 py-10 text-sm">Pilih barang di sebelah kiri.</div>
                ) : (
                  cart.map((item) => (
                    <div key={item.productId} className="flex justify-between items-start bg-slate-800/30 p-3 rounded-lg border border-slate-700/30">
                      <div className="flex-1 min-w-0 pr-2">
                        <h4 className="text-white text-sm font-medium truncate">{item.name}</h4>
                        
                        {/* DOKUMENTASI: Area input untuk mengubah harga per item secara manual */}
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <Input
                            ...
                            className="h-6 w-24 px-1.5 py-0 text-xs bg-slate-900 border-slate-700 text-emerald-400 font-mono focus:border-orange-500 focus:ring-1 focus:ring-orange-500"
                          />
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        <div className="flex items-center bg-slate-900 rounded-md border border-slate-700">
                          <button onClick={() => updateQuantity(item.productId, -1)} className="p-1 text-slate-400 hover:text-white hover:bg-slate-800 rounded-l-md transition-colors"><Minus className="h-3.5 w-3.5" /></button>
                          <span className="w-8 text-center text-xs font-medium text-white">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.productId, 1)} className="p-1 text-slate-400 hover:text-white hover:bg-slate-800 rounded-r-md transition-colors"><Plus className="h-3.5 w-3.5" /></button>
                        </div>
                        <button onClick={() => removeFromCart(item.productId)} className="text-red-400 hover:text-red-300 text-[10px] flex items-center gap-1"><Trash2 className="h-3 w-3" /> Hapus</button>
                      </div>
                    </div>
                  ))
                )}
              ...
              <div className="pt-4 mt-4 border-t border-slate-800/50 space-y-4">
                <div className="flex justify-between items-center"><span className="text-slate-400 text-sm">Total Belanja</span><span className="text-xl font-bold font-mono text-white">{formatRupiah(totalAmount)}</span></div>
                <div className="grid gap-2">
                  <Label className="text-slate-300 text-xs">Metode Pembayaran</Label>
                  <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as "lunas" | "kasbon")} className="w-full px-3 py-2 bg-slate-800/80 border border-slate-700/50 rounded-lg text-sm text-white focus:ring-orange-500/50">
                    <option value="lunas">Lunas (Langsung Bayar)</option>
                    <option value="kasbon">Kasbon (Hutang)</option>
                  </select>
                </div>
                {paymentMethod === "kasbon" ? (
                  <div className="grid gap-2"><Label className="text-slate-300 text-xs">Nama Pelanggan (Hutang)*</Label><Input value={personName} onChange={(e) => setPersonName(e.target.value)} className="bg-slate-800/80 border-slate-700 text-white text-sm" placeholder="e.g. Mas Budi" /></div>
                ) : (
                  <div className="grid gap-2"><Label className="text-slate-300 text-xs">Pilih Dompet (Masuk Uang)*</Label>
                    <select value={selectedWalletId} onChange={(e) => setSelectedWalletId(e.target.value)} className="w-full px-3 py-2 bg-slate-800/80 border border-slate-700/50 rounded-lg text-sm text-white focus:ring-orange-500/50">
                      <option value="" disabled>-- Pilih Dompet --</option>
                      {wallets.map((w) => <option key={w.id} value={w.id}>{w.name} ({formatRupiah(w.balance)})</option>)}
                    </select>
                  </div>
                )}
                <Button onClick={handleCheckoutRetail} disabled={isProcessing || cart.length === 0} className="w-full py-6 text-base font-bold bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white shadow-[0_0_15px_rgba(249,115,22,0.3)] transition-all">
      ...
      {/* --- TAB 2: SERVICES (JASA & LAYANAN PENGGANTI ADD INCOME) --- */}
      {activeTab === "services" && (
        <div className="w-full bg-slate-900/40 backdrop-blur-sm border border-slate-800/50 rounded-xl p-4 md:p-6 md:w-3/4 lg:w-2/3 mx-auto">
          <form className="space-y-6" onSubmit={handleServiceSubmit}>
            <div className="grid gap-2">
              <Label className="text-slate-300">Pilih Jenis Layanan/Jasa</Label>
              <select value={transactionType} onChange={(e) => setTransactionType(e.target.value as TransactionType)} className="w-full px-3 py-3 bg-slate-800 border border-emerald-500/30 rounded-lg text-white font-medium focus:ring-emerald-500/50">
                {serviceTypes.map((type) => <option key={type} value={type}>{type}</option>)}
              </select>
            </div>

            {/* RENDER FORM BERDASARKAN JENIS LAYANAN (DARI FILE PAGE.TSX LAMA) */}
            
            {transactionType === "Electronic Service" && (
              <div className="grid gap-4 bg-slate-800/30 p-4 rounded-lg border border-slate-700/50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="grid gap-2"><Label className="text-slate-300">Biaya Jasa (Rp)</Label><Input type="number" min="0" required value={serviceFee} onChange={(e) => setServiceFee(e.target.value)} className="bg-slate-900 border-slate-700 text-white" /></div>
                  <div className="grid gap-2"><Label className="text-slate-300">Sparepart (Opsional)</Label>
                    <select value={sparepartProductId} onChange={(e) => setSparepartProductId(e.target.value)} className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-md text-white text-sm">
            ...
            {transactionType === "Money Transfer" && (
              <div className="grid gap-4 bg-slate-800/30 p-4 rounded-lg border border-slate-700/50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="grid gap-2"><Label className="text-slate-300">Jumlah Transfer (Rp)</Label><Input type="number" required value={serviceAmount} onChange={(e) => setServiceAmount(e.target.value)} className="bg-slate-900 border-slate-700 text-white" /></div>
                  <div className="grid gap-2"><Label className="text-slate-300">Biaya Admin (Rp)</Label><Input type="number" required value={serviceAdminFee} onChange={(e) => setServiceAdminFee(e.target.value)} className="bg-slate-900 border-slate-700 text-white" /></div>
                  <div className="grid gap-2"><Label className="text-slate-300">Dompet Pengirim (Saldo Berkurang)*</Label>
                    <select value={serviceSourceWalletId} onChange={(e) => setServiceSourceWalletId(e.target.value)} className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-md text-white text-sm"><option value="">-- Pilih --</option>{wallets.map(w => <option key={w.id} value={w.id}>{w.name} ({formatRupiah(w.balance)})</option>)}</select>
                  </div>
                  <div className="grid gap-2"><Label className="text-slate-300">Dompet Penerima Uang Pelanggan*</Label>
                    <select value={serviceDestWalletId} onChange={(e) => setServiceDestWalletId(e.target.value)} className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-md text-white text-sm"><option value="">-- Pilih --</option>{wallets.map(w => <option key={w.id} value={w.id}>{w.name} ({formatRupiah(w.balance)})</option>)}</select>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="grid gap-4 bg-slate-800/30 p-4 rounded-lg border border-slate-700/50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="grid gap-2"><Label className="text-slate-300">Metode Pembayaran</Label>
                    <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as "lunas" | "kasbon")} className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-md text-white text-sm">
                      <option value="lunas">Lunas (Tunai/Transfer)</option><option value="kasbon">Kasbon (Hutang)</option>
                    </select>
                  </div>
                  {paymentMethod === "kasbon" ? (
                    <div className="grid gap-2"><Label className="text-slate-300">Nama Pelanggan (Hutang)*</Label><Input value={personName} onChange={(e) => setPersonName(e.target.value)} className="bg-slate-900 border-slate-700 text-white" /></div>
                  ) : (
                    <div className="grid gap-2"><Label className="text-slate-300">Dompet Tujuan (Uang Masuk)*</Label>
                      <select value={serviceDestWalletId} onChange={(e) => setServiceDestWalletId(e.target.value)} className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-md text-white text-sm"><option value="">-- Pilih --</option>{wallets.map(w => <option key={w.id} value={w.id}>{w.name} ({formatRupiah(w.balance)})</option>)}</select>
                    </div>
                  )}
                </div>
            ...
            <Button type="submit" disabled={isProcessing} className="w-full py-6 text-base font-bold bg-gradient-to-r from-emerald-500 to-emerald-600 text-white shadow-[0_0_15px_rgba(16,185,129,0.4)]">
